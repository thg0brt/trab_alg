...
#include <string.h>
int main(){
	int a, i, n, n_, A, B, c=1, x=0, countX; //declaração de variaveis inteiras
	...
	for(a=0; a<n;a++) vetX[a]=-1; //seta todos os valores de vetX como -1
	...
	while(n){ //repete o codigo enquanto n seja maior que 0
		...
		int i;
		for(i=0; i<c; i++){ //repete o codigo enquanto i seja menor que c
    ...
    for(i=0; i < n_; i++){
        printf("X%d = %.4f\n", i+1, vetX[i]);//imprime o resultado do X atual
    }
